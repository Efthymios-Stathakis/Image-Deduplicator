...
class ImageDeduplicator():
    ...
    def __init__(self, rel_path='/images/'):
        """
        Initialize the class and some attributs
...
def run():
    ...
    image_deduplicator = ImageDeduplicator(path_params)
    
    lof_images = params['lof_images']
    check_all  = params['check_all']
    if check_all == 'Yes':
        lof_images = image_deduplicator._lof_filenames
    
    for img in lof_images:
        lof_duplicates = image_deduplicator._find_duplicates(img)
